package com.example.jp.ui.theme

import androidx.compose.ui.graphics.Color

val TextWhite = Color(0xffeeeeee)
val DeepDark = Color(0xFF1B1B1B)
val ButtonDarkOrange = Color(0xFF2B201A)
val ButtonDarkOut = Color(0xFF232324)
val LightRed = Color(0xfffc879a)
val AquaBlue = Color(0xff9aa5c4)
val Red = Color(0xFFBD0404)
val TextGray = Color(0xFF6D7075)
val TextSelectedOrange = Color(0xFFAC4626)
val Orange = Color(0xFFE0562A)
val LighterDark = Color(0xFF222222)
val Beige3 = Color(0xfff9a27b)
val LightGreen1 = Color(0xff54e1b6)
val LightGreen2 = Color(0xff36ddab)
val LightGreen3 = Color(0xff11d79b)
val BlueViolet1 = Color(0xffaeb4fd)
val BlueViolet2 = Color(0xff9fa5fe)
val BlueViolet3 = Color(0xff8f98fd)